function [U_best, out]= RADA_RGD4CM(U, opts, H, lambda, Y)
...
F = sum(U.*HU,"all") + sum(U.*Y,"all") - lambda/2*norm(Y,'fro')^2 - beta/2*norm(Y-Y_now,'fro')^2;
G = 2*HU + Y;
    ...
    for nupdateU = 1:npU % npU times RGD to update U
        ...
        while 1
            ...
            if F <= Cval - zeta*deriv || nls >= 5
                ...
                f = sum(U.*HU,"all") + sparseparameter * sum(abs(U),"all");
                F_hist(itr) = f;
                nls_hist(itr) = nls;
