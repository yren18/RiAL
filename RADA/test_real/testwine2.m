...
% nrunresults.ADMM(nrunindex,:)=[function_multi_val_ADMM,t11, out_ADMM.itr];
%% ARPGDA
...
% nrunresults.DSGM(nrunindex,:)=[out_DSGM.F_best,t6,out_DSGM.itr];
...
% fprintf('NMI & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ \\\\ \n',...
%     Score_AMGA2, Score_ARGbb2y, Score_ARGbb, Score_DSGM, Score_ADMM);
% fprintf('$\\Phi$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ \\\\ \n',...
%     out_RADAPGD.F_best, out_RADARGD.F_best, out_ARPGDA.F_best, out_DSGM.F_best, out_NADMM.F_best);
% fprintf('cpu & $%.0f$ & $%.0f$ & $%.0f$ & $%.0f$ & $%.0f$ \\\\ \n',...
%     out_RADAPGD.itr, out_RADARGD.itr, out_ARPGDA.itr, out_DSGM.itr, out_NADMM.itr);
% fprintf('iter & $%.2f$ & $%.2f$ & $%.2f$ & $%.2f$ & $%.2f$ \\\\ \n',...
%     t_RADAPGD, t_RADARGD, t_ARPGDA, t_DSGM, t_NADMM);
