...
path = strcat(pwd,'/experiments/results/CM/');
filename = 'CM_May13_data_v1.mat';
...
function [Result, out_ManALM] = RunRep(n, m, r, mu, start, reps, sigma0, sigma0_SD, sparse_param, inner_min_itr, inner_mxitr)
    ...
    for num = start:reps
        ...
        t_RADA = 0;
        t = tic;
        [U_RADA, out_RADA]= RADA_PGD4CM(phi_init, opts, -A, lambda_RADA, Y0);
        t_RADA =t_RADA +toc(t);

        fprintf('                    RADA-PGD & itr=%4d & f=%.8f & time=%fs & sparse=%.4f \\\\ \n',out_RADA.itr, out_RADA.F_best,...
            t_RADA, out_RADA.sparse);
        T5 = [T5; t_RADA];
        F5 = [F5; out_RADA.F_best];
        I5 = [I5; out_RADA.itr];
        S5 = [S5; out_RADA.sparse * 100];
    end
    ...
    fprintf(1, 'RADA-PGD:   time = %.3fs,  sparsity = %.2f,  loss = %.4f, iter = %.4f\n', mean(T5), mean(S5), mean(F5),mean(I5));
    ...
    Result = [mean(F2), mean(F0), mean(F1), mean(F3), mean(F4), mean(F5);
            mean(V2), mean(V0), mean(V1), mean(V3), mean(V4), mean(V5);
            mean(I2), mean(I0), mean(I1), mean(I3), mean(I4), mean(I5);
            mean(T2), mean(T0), mean(T1), mean(T3), mean(T4), mean(T5);
            mean(S2), mean(S0), mean(S1), mean(S3), mean(S4), mean(S5);
            mean(TI2), mean(TI0), mean(TI1), mean(TI3), mean(TI4), mean(TI5)];
