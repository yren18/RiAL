function [X, out]= OptStiefelGBB_modified(X, fun, opts, varargin)
%-------------------------------------------------------------------------
...
if ~isfield(opts, 'details');   opts.details = 0; end
if ~isfield(opts, 'tiny');      opts.tiny = 1e-13; end
if ~isfield(opts, 'stop_criterion');      opts.stop_criterion = 1; end
if ~isfield(opts, 'subproblem_c1');      opts.subproblem_c1 = 1e-2; end
if ~isfield(opts, 'subproblem_c2');      opts.subproblem_c2 = 1e-3; end
if ~isfield(opts, 'inner_min_itr');      opts.inner_min_itr = 15; end
...
crit    = ones(nt, 3);
tiny    = opts.tiny;
stop_criterion = opts.stop_criterion;
...
if opts.details == 1
	out.record_time = [toc];
	%out.record_kkt  = [ max(max(abs(projG))) / (nrmG + 1) ];
	out.record_kkt  = [ nrmG ];
end
...
for itr = 1 : opts.mxitr
	...
	if opts.details == 1
		out.record_time = [out.record_time toc];
%		out.record_kkt  = [out.record_kkt  max(max(abs(projG))) / (nrmG + 1) ];
		out.record_kkt  = [out.record_kkt  nrmG ];
	end
    ...
    if (record >= 1 && (mod(itr, 100) == 0 || itr==2))
        fprintf('%4d  %3.2e  %4.3e  nrmG = %3.2e  %3.2e  %3.2e  %2d\n', ...
    ...
    switch stop_criterion
        ...
        case 2
            c = min([subproblem_c1*nrmG0, subproblem_c2]);
            sufficent_decrease = (F-F0 < -c*nrmG0);
            new_stopping_criterion = ((sufficent_decrease) && (itr >= inner_min_itr));
            if ( XDiff < xtol && FDiff < ftol ) || nrmG < gtol || all(mcrit(2:3) < 10*[xtol, ftol]) || new_stopping_criterion
                out.msg = 'converge';
                cstop = 1;
            end
    end        
