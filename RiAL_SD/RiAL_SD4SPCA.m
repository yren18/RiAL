function [U, out]= RiAL_SD4SPCA(U, opts, A)
    %% load input
    ...
    for itr = 1:mxitr
        ...
        [U, info] = OptStiefelGBB_modified(U, @fun, inner_opts, A, Y, sigma, sparse);
        AU = A*U;
        curF = -sum(AU.*AU,"all") + sparse*sum(abs(U),'all');
        F_hist(itr) = curF;
        ...
        etaP = norm(U-P,'fro')/(1+norm(U,'fro')+norm(P,'fro'));
